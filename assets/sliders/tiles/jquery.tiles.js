    ...
    TilesSlider.prototype = {
        _init: function () {
            ...
            if (!self.$images.length)
                return false;
            self.$container.addClass("tiles-slider-wrap tiles-slider-" + o.effect + " tiles-slider-s" + o.cssSpeed);
            self.$images.each(function () {
                self._generateTiles($(this))
            });
        ...
        _addNav: function () {
            var self = this,
                    ...
                    links = [],
                    $links, thumb, $next = $('<a href="#" class="tiles-next">Next &raquo;</a>'),
            ...
            for (var i = 1; i < self.$wraps.length + 1; i++) {
                thumb = '<span><img src="' + self.$images.eq(i -
                        1).attr("src") + '"/></span>';
                links.push('<a href="#" class="tiles-nav-item ' + (o.bullets ? "tiles-bullet" : "tiles-pagination") + '">' + (!o.bullets ? i : "") + (o.thumbs ? thumb : "") + "</a>")
            }
            $links = $(links.join(""));
            ...
            $links.first().addClass("tiles-nav-active");
            $next.click(function () {
                self.next()
            });
            $prev.click(function () {
            ...
            $links.add($next).add($prev).click(function (e) {
                e.preventDefault()
            });
            if (o.navWrap)
                $nav.append($links);
        ...
        _updateNav: function () {
            if (this.opts.nav)
                this.$navLinks.removeClass("tiles-nav-active").eq(this._getCurrentIdx()).addClass("tiles-nav-active")
        },
        _setupDescriptions: function () {
            ...
            this.$container.find("p").each(function () {
                $(this).wrapAll('<div class="tiles-description ' + ($(this).is(":empty") ? "tiles-description-empty" : "") + '"/>')
            })
        },
        _showOrHideDescription: function (toggle) {
            this.$descriptions.removeClass("tiles-description-active").eq(this._getCurrentIdx()).toggleClass("tiles-description-active", toggle)
        },
        _generateTiles: function ($img) {
            ...
            $tiles = $(tiles.join(""));
            $tiles.addClass("tiles-x" + o.x + " tiles-y" + o.y).filter(":odd").addClass("tiles-odd").end().filter(":even").addClass("tiles-even");
            $tiles.css({
        ...
        _animateTiles: function ($wrap, callback, fn) {
            ...
            if (o.reverse)
                range = range.reverse();
            if (o.limit)
                o.reverse ? range.splice(0, limit) : range.splice(limit, range.length);
            $.each(range, function (i, tile) {
                var theTile =
                        $tiles.eq(tile),
                        theDelay = i * delay;
                setTimeout(function () {
                    theTile.addClass(self.klassAnim)
                }, theDelay);
                if (o.rewind) {
                    theDelay += o.cssSpeed / (100 / o.rewind);
                    setTimeout(function () {
                        theTile.removeClass(self.klassAnim)
                    }, theDelay)
                }
            });
            setTimeout(function () {
        ...
        stop: function () {
            $.each(this.timeouts, function (i, v) {
                clearTimeout(v)
            });
